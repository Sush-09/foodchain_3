...
class ItemsPurchsedByRetailer extends Component {
    ...
    const product_length = await factory.methods.getProductsPurchasedLength().call();
    ...
    const product_data = this.props.products.filter((product) => {
        if (product.retailerId == this.props.address){
            return product;
        }
    });
...
export default ItemsPurchsedByRetailer;